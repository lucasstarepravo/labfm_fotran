...
subroutine SPARSE_PBICGSTAB(A,ija,x,B,n,nmax,itermax,tol,iter,residial)
  ...
  real(rkind) ::  temp,ro,ro0,alpha,omega,beta 
  real(rkind),dimension(:),allocatable :: r,r_bar,v,AX,p,s,t
  real(rkind),dimension(:),allocatable :: p_inv,s_inv,M 
  real(rkind) ::  eps_small
  real(rkind) :: norm2_t,norm2_r,norm2_r0,norm2_s,norm2_s0 
...
!     M(i)=1.0d0/min(A(i),-eps_small)
     M(i)=sign(1.0d0,A(i))/max(abs(A(i)),eps_small)
     t(i)=0.0d0
  enddo
  !$OMP END PARALLEL DO
...
subroutine sparse_atime(A,ija,x,b,n,nmax)
  ...
  real(rkind) :: b(n),bb,x(n)      !!    vecs are dkind
  integer :: i,k
