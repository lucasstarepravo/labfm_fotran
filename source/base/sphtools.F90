module sphtools
  ...
  private
  public :: wab,fac
!! Note. kernel normalisation (e.g. 7/478/pi/h^2) much less important in consistency
!! correction methods. Often omitted.

!! CHOICE OF "SPH" KERNELS...
!! 0 - Constant
!! 1 - Wendland C2
!! 2 - Gaussian (normalised as in Labfm paper).
!! 3 - Gaussian 4th order
!! 4 - Wendland C6
...
!! Gaussian ---------------------------------------------------------------------------------------
  function wab(qq) result(factemp) 
    ...
    qq3=3.0*qq
!    factemp = (9.0/(pi))*exp(-qq3**2.0)
    factemp = (1.0/(h0*h0*pi))*exp(-qq**2.0)    
  end function wab
  function fac(qq) result(factemp) 
    real(rkind), intent(in) :: qq
    real(rkind) ::  factemp,qq3
    qq3=3.0*qq
    factemp = (-1.0/(pi*h0))*2.0*qq3*exp(-qq3**2.0)
  end function fac
  ...
  function wab(qq) result(factemp) 
    ...
    real(rkind) ::  factemp,qq3
    qq3=3.0*qq
    factemp = (1.0/(h0*h0*pi))*(1.0d0-0.5d0*qq**2.0d0)*exp(-qq**2.0)    
  end function wab
  function fac(qq) result(factemp)  !! Just 2nd order here, as not using
    real(rkind), intent(in) :: qq
    real(rkind) ::  factemp,qq3
    qq3=3.0*qq
    factemp = (-1.0/(pi*h0))*2.0*qq3*exp(-qq3**2.0)
...
!! Wendland C6 ------------------------------------------------------------------------------------
  ...
  function fac(qq) result(factemp)  !! Wendland C6 
    ...
    factemp = 0.0d0
    if(qq.le.2.0d0.and.qq.gt.0.0d0) then
       factemp = -(429.0/(7168.0*pi*h0))*qq*(8.0*qq2 + 7.0*qq + 2.0)*qq7
    end if
  end function fac
#elif kernel==5
!! ------------------------------------------------------------------------------------------------
!! Wendland C6, modified to 4th order (Abouzied's kernel) -----------------------------------------
  ...
  function fac(qq) result(factemp) 
    ...
    factemp = 0.0d0
    if(qq.le.2.0d0.and.qq.gt.0.0d0) then
       factemp = -(2145.0/(362.0*pi*h0))*qq*qq7*(1768.d0*qq4+1190.d0*qq3-1172.d0*qq2-1323.d0*qq-378.d0)/5632.d0
    end if
  end function fac
#elif kernel==6
!! EMPTY FOR NOW
